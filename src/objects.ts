...
export function isValid(question: Question, answer: string): boolean {
    ...
    if (question.type === "multiple_choice_question") {
        const checkAnswer = question.options.findIndex(
            (ans: string): boolean => ans === answer
        );
        if (checkAnswer !== -1) return true;
    }
    return false;
}
...
export function renameQuestion(question: Question, newName: string): Question {
    const ques = { ...question, name: newName };
    return ques;
}
...
export function publishQuestion(question: Question): Question {
    if (question.published) {
        const ques = { ...question, published: false };
        return ques;
    } else {
        const ques = { ...question, published: true };
        return ques;
    }
}
